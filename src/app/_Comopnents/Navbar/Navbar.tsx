...
import { useRouter, usePathname } from "next/navigation"; // ⬅️ هنا
...
export default function Navbar() {
  ...
  const pathname = usePathname(); // ⬅️ هنا
          ...
          {status === "authenticated" && (
            <>
              <Link href="/washlist"  className={`relative ${linkClass("/washlist")}`}>
                ❤️
                {washlistCount > 0 && (
                  <span className="absolute -top-2 -right-2 text-xs bg-red-500 text-white rounded-full px-1">{washlistCount}</span>
                )}
              </Link>
              ...
              <span onClick={handleLogout} className="hidden md:block px-4 py-2 text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                Logout
              </span>
            </>
          )}
      ...
      <ul className="flex flex-col p-4 space-y-2 font-medium text-gray-700 dark:text-gray-200">
        {status === "authenticated" && (
          <>
            ...
            <li><span onClick={handleLogout}  className="block py-2 px-2 text-red-600 cursor-pointer">Logout</span></li>
          </>
        )}
